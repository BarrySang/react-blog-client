...
import { formatNumber, getBlog } from "../lib/lib-functions/lib-functions"

// function Blog ({user, content, likes, comments, title, id}) {
function Blog ({blog, blogs}) {
    const {blogId} = useParams()
    // const [blogComponent,setBlogComponent] = useState(blog)

    // console.log(blogComponent)

    if (!blog && blogId) {
        
        blog = getBlog(blogId, blogs)
    ...
    if (blog) {
        ...
        return (
            <div className="blog-container">
                ...
                <p className="blog-title"><Link to={`/blogs/${blog.id}`} className="blog-title-link">{blog.title}</Link></p>
                {
                    <p className="blog-content">{blog.content}</p>
                }
                <div className="blog-response-container">
